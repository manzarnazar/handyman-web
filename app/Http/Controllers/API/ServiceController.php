    ...
    public function getServiceDetail(Request $request)
    {
        ...
        if (auth()->user() !== null) {
            if (auth()->user()->hasRole('admin')) {
                $service = Service::where('service_type', 'service')->withTrashed()->where('status', 1)->with('providers', 'category', 'serviceRating', 'serviceAddon', 'translations', 'zones','servicePackage','shops')->findorfail($id);
            } else {
