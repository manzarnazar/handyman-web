...
const props = defineProps(['service','coupons','taxes','user_id','availableserviceslot','serviceaddon','googlemapkey','wallet_amount','shop_id','shop_list']);
...
const config = {
  ...
  minDate: 'today', // Disables past dates, keeps today as the minimum selectable date
  maxDate: maxDate.value, // Ensures users can select until service's `end_at` date
  onChange: (selectedDates, dateStr, instance) => {
    ...
    if (selectedDate < currentDate) {
      ...
      instance.clear(); // Clear the selected date if it's invalid
    }
  }
};
...
const validationSchema = yup.object({
  ...
  const timePart = value.split(' ')[1]; // extract HH:mm
