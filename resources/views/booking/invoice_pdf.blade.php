            ...
            <div style="overflow: hidden;">
                <div style="float: left; display: inline-block;">
                    {{-- <img style="height: 32px; width: 32px; "
                        ...
                        <?php
                        // Get the site logo dynamically using the AppSetting model
                        $appSetting = \App\Models\AppSetting::first();
                        $logoUrl = getSingleMedia($appSetting, 'site_logo');

                        // If we have a valid logo URL, fetch and encode it
                        if ($logoUrl) {
                            // Get the full path to the logo
                            $imagePath = str_replace(url('/'), public_path(), $logoUrl);

                            // Check if the file exists before trying to read it
                            if (file_exists($imagePath)) {
                                $imageData = base64_encode(file_get_contents($imagePath));
                                echo '<img src="data:image/png;base64,' . $imageData . '" style="height: 32px; width: 32px;">';
                            } else {
                                // Fallback to default logo if the custom one doesn't exist
